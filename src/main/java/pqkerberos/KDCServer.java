...
/**
 * KDCServer — Key Distribution Center for PQ-Kerberos.
 *
 * Runs two listeners:
 *   AS port 8888  — Authentication Service (issues TGTs)
 *   TGS port 8889 — Ticket Granting Service (issues service tickets)
 *
 * CHANGE from original: addKerberosUser() is now public so SystemDaemon
 * can register users loaded from /etc/pqkerberos/users.conf at runtime.
 * Also added getServiceKey() for PAMAuthDaemon to hand the correct key to
 * FileService (avoids the double-generate race in getServiceKeyForDemo).
 */
public class KDCServer {
    ...
    public void initialize() throws GeneralSecurityException {
        ...
        // Default demo users — additional users can be added via addKerberosUser()
    ...
    /** Register a Kerberos principal. Safe to call before or after start(). */
    public void addKerberosUser(String principalId) {
        if (!userDatabase.containsKey(principalId)) {
            registerUser(principalId);
            System.out.println("[KDC] Registered user: " + principalId);
        }
    }
    ...
    private void handleASRequest(Socket clientSocket) {
        try {
            ...
            String replayKey = request.clientId + ":" + request.timestamp + ":"
                    + Arrays.toString(request.nonce);
    ...
    private ASResponse buildASResponse(ASRequest request) throws Exception {
        ...
        SecretKey kemDerivedKey = PQCrypto.deriveAESKey(kemResult.sharedSecret,
                "as-session-key-wrap".getBytes());
        response.encryptedSessionKey = PQCrypto.encrypt(sessionKeyBytes, kemDerivedKey);

        // Sign the response — covers ALL fields so nothing can be tampered
    ...
    private void handleTGSRequest(Socket clientSocket) {
        try {
            ...
            if (tgtContents == null || tgtContents.isExpired()) {
                sendError(clientSocket, "TGT invalid"); return;
            }
            ...
            if (auth == null || auth.isExpired()) {
                sendError(clientSocket, "Authenticator invalid"); return;
            }
            if (!auth.clientId.equals(tgtContents.clientId)) {
                sendError(clientSocket, "Identity mismatch"); return;
            }
            ...
            if (!serviceDatabase.containsKey(request.serviceName)) {
                sendError(clientSocket, "Unknown service"); return;
            }

            TGSResponse response = buildTGSResponse(request, tgtContents);
            MessageIO.send(clientSocket, response);
            System.out.println("[TGS] Service ticket: " + tgtContents.clientId
                    + " → " + request.serviceName);
    ...
    private TGSResponse buildTGSResponse(TGSRequest request, TicketInner tgtContents)
            throws Exception {
        ...
        TicketInner inner = new TicketInner(tgtContents.clientId, serviceSessionKeyBytes,
                response.timestamp, response.expiryTimestamp);
        ...
        SecretKey kemDerivedKey = PQCrypto.deriveAESKey(kemResult.sharedSecret,
                "tgs-service-session-key-wrap".getBytes());
    ...
    // ── Public accessors for SystemDaemon / AttackSimulator ──────────────

    public PublicKey getSigningPublicKey() { return kdcSigningKeyPair.getPublic(); }

    /**
     * Register a service and return its long-term key.
     * Safe to call multiple times — returns the SAME key on repeated calls
     * (fixes the original getServiceKeyForDemo double-generate race).
     */
    public byte[] getServiceKeyForDemo(String serviceName) {
        serviceDatabase.computeIfAbsent(serviceName, k -> {
            byte[] key = new byte[32];
            new SecureRandom().nextBytes(key);
            return key;
        });
    ...
    private AuthenticatorInner decryptAuthenticator(EncryptedAuthenticator auth, SecretKey key) {
        try {
            return MessageIO.fromBytes(PQCrypto.decrypt(auth.encryptedData, key),
    ...
    private PublicKey deserializeKEMPublicKey(byte[] keyBytes) throws Exception {
        ...
        return KeyFactory.getInstance(PQCrypto.KEM_ALGORITHM, PQCrypto.PROVIDER_PQC)
