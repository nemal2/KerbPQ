...
/**
 * FileService — A simple service that accepts PQ-Kerberos authenticated requests.
 *
 * The service:
 *   - Holds a long-term symmetric key shared with the KDC out-of-band
 *   - Uses this key to decrypt service tickets
 *   - Validates the authenticator to prevent replay attacks
 *   - Provides mutual authentication by proving it decrypted the ticket
 *
 * The service NEVER contacts the KDC during authentication —
 * this is a key property of Kerberos (scalability + availability).
 * The ticket itself carries everything the service needs.
 */
